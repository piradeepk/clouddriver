...
public class EcsCreateServerGroupDescriptionValidator extends CommonValidator {
  ...
  private void validateTargetGroupMappings(
    ...
    if (createServerGroupDescription.getTargetGroupMappings() != null) {
      ...
      boolean isInputContainer =
          StringUtils.isNotBlank(createServerGroupDescription.getDockerImageAddress());
      ...
      for (CreateServerGroupDescription.TargetGroupProperties targetGroupProperties :
        ...
        boolean hasContainer = StringUtils.isNotBlank(targetGroupProperties.getContainerName());

        if (hasTargetGroup && !hasContainer && !isInputContainer) {
          rejectValue(errors, "containerName", "not.nullable");
        } else if (!hasTargetGroup && hasContainer) {
          rejectValue(errors, "targetGroup", "not.nullable");
        }
